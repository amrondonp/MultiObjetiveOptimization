...
public class Promethee {
  ...
  private double compare(Alternative a, Alternative b) {
    ...
    for (Criterion c : criteria) {
      try {
        ans +=
          c.compute(a.getValues().get(i), b.getValues().get(i)) * c.getWeight();
        tot += c.getWeight();
        i++;
      } catch (Exception ex) {
  ...
  public void computeFlows() {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        if (i == j) preferenceIndex[i][j] = 0.0; else preferenceIndex[i][j] =
          compare(alternatives.get(i), alternatives.get(j));
      }
    }
